...
struct ViewController: UIViewRepresentable {
    ...
    static var planetary_offset: CGFloat = 3.0
    ...
    func generatePlanet(planet: PlanetModel) -> SCNNode {
        ...
        let sign_factor = planet.id < manager.currentPlanet.id ? -1.0 : 1.0
        let originZ = -4.0 // arbritarily looks nice at this z offset
        let actual_id_offset = CGFloat(planet.id - manager.currentPlanet.id)
        let zedOff: CGFloat = (sign_factor * originZ) - (actual_id_offset * ViewController.planetary_offset)
